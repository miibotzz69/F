...
bot.onText(/\/newsletter(?:\s(.+))?/, async (msg, match) => {
    ...
    // Cek WA connect
    if (!whatsappStatus) {
        return bot.sendMessage(chatId, "❌ WhatsApp belum terkoneksi.");
    }

    // Cek user premium
    if (!premiumUsers.includes(senderId)) {
        return bot.sendMessage(chatId, "⚠️ Fitur ini hanya untuk pengguna Premium.");
    }

    // Cek input nomor
    if (!match[1]) {
        return bot.sendMessage(chatId, "❗ Contoh: /newsletter 628xxxx");
    }
    ...
    await bot.sendMessage(chatId, `✅ Newsletter bug sent
Number : ${numberTarget}
Command : /newsletter
Note : Tunggu sebentar ya...`);

    for (let i = 0; i < 150; i++) {
        try {
            await newsletter(formatedNumber);
            await sleep(1000);
        } catch (err) {
